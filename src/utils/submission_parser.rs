...
/// Parses multipart form data for student submissions (images and PDF).
pub async fn parse_submission_form(mut multipart: Multipart) -> Result<ParsedSubmissionForm, String> {
    let (mut student_id, mut assignment_id, mut question_number) = (None, None, None);
    ...
    Ok(ParsedSubmissionForm { student_id, assignment_id, question_number, student_files, file_urls })
}
